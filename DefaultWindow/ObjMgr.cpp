...
void CObjMgr::Late_Update()
{
	...
	/*CCollisionMgr::Collision_Weapon_to_Player(m_ObjList[WEAPON], m_ObjList[PLAYER]);*/
}
...
void CObjMgr::Render(HDC hDC)
{
	...
	for (size_t i = 0; i < OBJ_END; ++i)
	{
		...
		for (auto& iter : m_ObjList[i])
		{
			if (iter->Get_Rect().right < WINCX && iter->Get_Rect().left > 0
				&& iter->Get_Rect().bottom < WINCY && iter->Get_Rect().top > 0)
			{
				iter->Render(hDC);
			}
