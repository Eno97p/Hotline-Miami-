...
void CTileMgr::Render(HDC hDC)
{
	...
	int		iMaxX = CameraX +  WINCX /  2 /*+ TILECX*/;
	int		iMaxY = CameraY +  WINCY /  2 /*+ TILECY*/;

	int		iCullX = CameraX - WINCX /  2 + TILECX;
	int		iCullY = CameraY - WINCY /  2 + TILECY;
...
void CTileMgr::Save_DataT()
{
	HANDLE	hFile = CreateFile(L"../Data/TTile.dat", GENERIC_WRITE, NULL, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (INVALID_HANDLE_VALUE == hFile)
		return;
	int		m_iXNum(0), m_iYNum(0);
	DWORD	dwByte(0);
...
void CTileMgr::Load_DataT()
{
	HANDLE	hFile = CreateFile(L"../Data/TTile.dat", GENERIC_READ, NULL, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (INVALID_HANDLE_VALUE == hFile)
		return;
	INFO	tInfo{};
	int		m_iXNum(0), m_iYNum(0);
